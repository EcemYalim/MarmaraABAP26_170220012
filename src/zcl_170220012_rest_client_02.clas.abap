CLASS zcl_170220012_rest_client_02 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_170220012_rest_client_02 IMPLEMENTATION.
...
*    io_out->write( user_delete( ) ).
  ...
  ENDMETHOD.
    ...
    rv_text = execute_request(
        iv_method   = 'GET'
        iv_url      = gc_base_url && '/user/demo_170220012'
    ).
  ENDMETHOD.
    ...
    rv_text = execute_request(
        iv_method   = 'PUT'
        iv_url      = gc_base_url && '/user/deneme'
        iv_body     = build_user_json(  )
    ).
  ENDMETHOD.
  ...
  METHOD create_http_client.
    TRY.
        ...
        CATCH cx_http_dest_provider_error INTO DATA(lo_http_dest_provider_error).
            DATA(lv_text_02) = lo_web_http_client_error->get_text(  ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD build_user_json.
    ...
    rv_json =
      '{' &&
      '"id": 19,' &&
      '"username": "demo_170220012",' &&
      '"firstName": "deneme",' &&
      '"lastName": "deneme",' &&
      '"email": "170220012@example.com",' &&
      '"password": "password",' &&
      '"phone": "1111111111",' &&
      '"userStatus": 1' &&
      '}'.
